...
int 
ecall_wamr_execute(uint8_t* input_file, int input_file_size, int argc, char** argv, char* errorbuf, char* output)
{
    // Initialize the whole environment here

    // init runtime
    uint64 args_init_runtime[] = {(uint64) false, 4};
    handle_cmd_init_runtime(args_init_runtime, sizeof(args_init_runtime));

    // set verbose level, default = 2
    uint64 args_log_level[] = {4};
    handle_cmd_set_log_level(args_log_level, sizeof(args_log_level));

    // load module and get module ref
    // error size: 128
    uint64 args_load_module[] = {(uint64_t)(uintptr_t)input_file, input_file_size, (uint64_t)(uintptr_t)errorbuf, 128};
    handle_cmd_load_module(args_load_module, sizeof(args_load_module));

    void* wasm_module = (void *)(uintptr_t)args_load_module[0];
    ...
    uint64 args_wasi_args[] = {(uint64_t)(uintptr_t)wasm_module, (uint64_t)(uintptr_t)dir_list, dir_list_size, (uint64_t)(uintptr_t)env_list, env_list_size, (uint64_t)(uintptr_t)argv, argc};
    handle_cmd_set_wasi_args(args_wasi_args, sizeof(args_wasi_args));

    // instantiate module and get instance ref
    uint64 args_instantiate_module[] = {(uint64_t)(uintptr_t)wasm_module, 16 * 1024, 16 * 1024, (uint64_t)(uintptr_t)errorbuf, 128};
    handle_cmd_instantiate_module(args_instantiate_module, sizeof(args_instantiate_module));
    void* wasm_module_inst = (void *)(uintptr_t)args_instantiate_module[0];

    // exec app main
    uint64 args_exec_app_main[16];
    args_exec_app_main[0] = (uintptr_t)wasm_module_inst;
    args_exec_app_main[1] = argc;
        for (int i = 0; i < argc; i++) {
        args_exec_app_main[i + 2] = (uintptr_t)argv[i];
    }
    handle_cmd_exec_app_main(args_exec_app_main, argc + 2);


    ocall_print("until now OK!\n");
