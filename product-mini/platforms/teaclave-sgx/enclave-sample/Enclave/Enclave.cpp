...
uint32 teaclave_open(wasm_exec_env_t exec_env, char* fid, int* dummy);
...
handle_cmd_init_runtime(uint64 *args, uint32 argc)
{
    ...
    /* export Teaclave pf APIs */
    static NativeSymbol native_symbols[] = { {
      "teaclave_open", // the name of WASM function name
      (void*)teaclave_open,    // the native function pointer
    ...
    if (!wasm_runtime_register_natives("env", native_symbols,
                                       n_native_symbols)) {
        LOG_ERROR("export Teaclave pf APIs failed.\n");
    }
    
    LOG_ERROR("attachment: %d, n_native_symbols: %d\n", native_symbols[0].attachment, n_native_symbols);
...
handle_cmd_load_module(uint64 *args, uint32 argc)
{
    ...
    enclave_module->wasm_file =
      (uint8 *)enclave_module + sizeof(EnclaveModule);
